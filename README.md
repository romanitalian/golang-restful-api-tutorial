...
I want to share with you, the way I learned GoLang, that allow me today to start developping RestFul APIs as I wanted.
To do so, what better way than developping a bar/brewery !
It's may sounds crazy but it's not, we will together understand the concepts behind this by serving beers.

![beer taps](https://image.ibb.co/jysRcc/beer_tap_2435408_1920.jpg)

## Introducing Golang
Before any beers, we need to introduce what is Golang.
Its development has began at Google in 2007 and was released in 2009.
...
It is one of the best for quick compilation, concurrency developments and parallele executions.
I did not made it up, this is one of the creator of the language, Rob Pike, that said : "You can compile and run a go program faster than some interpreters can even start".
More recently, It can also be used to build mobile apps on Android and iOS devices.

> This tutorial is not about learning Go syntax, if you want to do so, I truly recommand you to visit the websites that I suggest at the end of this tutorial.
...
## Building
 ...
 ### Prerequisites
What so obvious than installing Go on your machine. For this, nothing more easy, go to : https://golang.org/dl/ and follow the instructions depending of your OS.
*Personnally, I use a Linux distribution and I will use [Goland](https://www.jetbrains.com/go/) from JetBrains as IDE, but you can also use [Visual Source Code](https://code.visualstudio.com/) which have good support and plugins for Go Programming.*
...
There going to be 2 services, one for the bar, and another for the brewery. The customers will ask for beers to the bar and it will give beers to the customers.
...
> Here, stocks can be related to databases. For the sake of simplicity, this tutorial won't aboard part about working with databases, but I've linked some useful informations at the end of it.
...
We will using a third library for routing called [Mux](https://github.com/gorilla/mux). To install it, *if it's not already done*, just execute the command below :

```bash
$ go get -u github.com/gorilla/mux
```

Any Go program start with a file called ```main.go``` in the package ```main```, so let's create one at the root of our workspace, and fill it with basic instructions :
...
* Run ``` $ go run main.go```
* Run ``` $ go build ```. It's going to compile and create an executable. So then, run ```$ ./theGreenDragon ```

> Remember these commands, you will need them each time you want to execute your program.

You've now opened your bar, or at least a server on port ```8000```. Next step, we will adding routes and handlers
...
> Some of these routes should be restricted with authentication, but this is not the purpose in the tutorial, we will try to keep things simple.
...
Let's see, what does it look like with our project :
```go
func main(){
	fmt.Println("Welcome to the The Green Dragon !")
	router := mux.NewRouter()
...
You should see red right now :D. That's because we have to add the handlers functions. For that, I decided to create to more file ```barHandlers.go``` and ```breweryHandlers.go ```. And let's add tho them, the handlers of our routes.
...
Let's explain quickly how are composed these methods. There are 2 parameters, their names are quite evoking, the ```writer``` is about handling with the response and the ```request``` contains everything you have to know about... request.
...
### Data
We talk about beers, barrels and mugs but we don't know how to represent those in our application. It's time to see how to create types !
For this, I decided to create a file ```database.go``` which will be our database. So, it will contains the models and the lists we need.
...
```
I've created a little structure which represent the order. It contains the ID of the beer the customer wants to drink, the quantity and the money to pay the bar.
...
As you can see, I've also created 2 functions :
*  ```FindBeerByID``` in ```database.go``` in order to simulate a query to my database. We definitely should test if the return of the function is not empty but let's say it always return what we asked for.
* ```serveBeer``` in ```barHandlers.go``` : it checks if there is enough beer, if not, it will ask to brewery a new barrel by calling the path ```/brewery``` via POST method. Then it serves the beers and returns the mugs.
...
I skip the part about breaking a mug, there is nothing very interresting to see here. You can implements whatever you like. Personnaly, I added a counter that count every mug that broke. But we could imagine a limited number of mug where people have to give back their mugs after finishing their beers or something like that. I should be a great exercice for concurrency ;)
...
```
We could totally call directly the function ```ProduceBarrels()``` from ```OrderBarrels``` but I wanted to show how to create a HTTP request. It could be useful when it comes to request another service.
The ```ProduceBarrels``` function is quite easy. Each time we produce barrels, we create one barrel of each type of beer. Despite everything, we should think about checking that after producing the barrels, we have enough beer to satisfied the initial order, otherwise, we should produce new barrels until we fulfill the need. But once again, keep this tutorial simple.

### Final Chapter
You can now launch the program and try to get infos about beers, order one, produce barrels of beer by calling the API.
In this tutorial, I choosed to not explain how create simple CRUD operations over REST method using Golang. You will find easily one on the Internet. Personnaly, I wanted something a bit fun, with a different approch. Especially about being called and calling other services.
Note that all the code is available right [here](https://github.com/Riverside14/golang-restful-api-tutorial).
> Be aware that I add severals comments in the final project, If some of those lines above was not clear, you probably want to check it.
